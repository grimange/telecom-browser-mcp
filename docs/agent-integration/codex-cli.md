...
## Transport
- Integration surface for this guide: `stdio`
...
- Session-bound responses also expose adapter identity in `meta`:
...
- `adapter_target_mismatch`: Known APNTalk hosts fail closed if a different adapter is explicitly requested.
- `adapter_contract_unimplemented` / `selector_contract_missing` / `runtime_probe_unavailable`: APNTalk modernization paths intentionally fail closed until verified selectors and runtime probes are implemented.
