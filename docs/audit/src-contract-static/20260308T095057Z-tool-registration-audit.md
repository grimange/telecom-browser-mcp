...
- Mechanism:
  - Sets strict FastMCP argument model (`ArgModelBase.model_config.extra = "forbid"`).
  - Iterates `TOOL_NAMES`, resolves bound orchestrator handler, registers via `server.tool(name=tool_name)(handler)`.
- Schema style: inferred from bound handler signatures (no explicit manual `inputSchema` declarations in source).
- Wrapper: none (direct method registration).
...
- Current `src` has one canonical registration site and no synthetic wrapper registration.
- Public tool schema shape is signature-driven and strict.
