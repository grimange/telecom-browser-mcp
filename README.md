...
APNTalk now supports visible-UI login plus bounded runtime-bridge observation. When APNTalk emits a valid `window.__apnTalkTestBridge`, the adapter can truthfully support `get_registration_status`, `wait_for_registration`, `wait_for_ready`, `wait_for_incoming_call`, `get_peer_connection_summary`, and bounded visible-UI `answer_call`/`hangup_call` using the main softphone controls only. `get_store_snapshot` remains intentionally fail-closed.

For the current bounded APNTalk support matrix, quickstart, refusal boundaries, and bridge compatibility notes, see [docs/modernization/apntalk-release-handoff.md](/home/grimange/personal_projects/telecom-browser-mcp/docs/modernization/apntalk-release-handoff.md:1), [docs/usage/codex-agent-usage.md](/home/grimange/personal_projects/telecom-browser-mcp/docs/usage/codex-agent-usage.md:1), and [docs/apntalk-runtime-bridge-contract.md](/home/grimange/personal_projects/telecom-browser-mcp/docs/apntalk-runtime-bridge-contract.md:1).
